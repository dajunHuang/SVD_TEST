...
int main(int argc, char *argv[]) {
    ...
    std::vector<double> D(minmn, 0);
    std::vector<double> E(minmn, 0);
    std::vector<double> TAUQ(minmn, 0);
    std::vector<double> TAUP(minmn, 0);
    ...
    double *d_D = nullptr;
    double *d_E = nullptr;
    double *d_TAUQ = nullptr;
    double *d_TAUP = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_D), sizeof(double) * D.size()));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_E), sizeof(double) * E.size()));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_TAUQ), sizeof(double) * TAUQ.size()));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_TAUP), sizeof(double) * TAUP.size()));
    ...
    CUSOLVER_CHECK(cusolverDnDgebrd_bufferSize(cusolverH, m, n, &lwork));
    ...
    for(int i{0}; i < NUM_WARPUP; ++i)
    {
        ...
        CUSOLVER_CHECK(cusolverDnDgebrd(cusolverH, m, n, d_A, lda, d_D, d_E, d_TAUQ,
    ...
    CUDA_CHECK(cudaMemcpyAsync(D.data(), d_D, sizeof(double) * D.size(), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(E.data(), d_E, sizeof(double) * E.size(), cudaMemcpyDeviceToHost, stream));
    ...
    std::cout << "Cusolver BRD (double) Latency: " << time << " ms" << std::endl;
