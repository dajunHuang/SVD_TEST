...
int main(int argc, char *argv[]) {
    ...
    CUSOLVER_CHECK(cusolverDnDorgqr_bufferSize(cusolverH, m, n, n, d_A, lda, d_TAU, &ldworkw));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_work_w), sizeof(double) * ldworkw));
    ...
    CUSOLVER_CHECK(
        cusolverDnDorgqr(cusolverH, m, n, n, d_A, lda, d_TAU, d_work_w, ldworkw, devInfo));
    ...
    for (int i{0}; i < NUM_WARPUP; ++i) {
        ...
        launchKernel_copyLower(gridDim, blockDim, m, n, d_A, lda, d_Y, ldy);
        CUSOLVER_CHECK(
            cusolverDnDorgqr(cusolverH, m, n, n, d_A, lda, d_TAU, d_work_w, ldworkw, devInfo1));
    }
    ...
    std::cout << "m: " << m << ", n: " << n << ", Cusolver QRF + ORGQR (double) Latency: " << time
