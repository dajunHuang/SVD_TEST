...
int main(int argc, char *argv[]) {
    ...
    // CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_C1), sizeof(double) * ldc * n));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_C2), sizeof(double) * ldc * n));
    ...
    std::vector<double *> h_Aarray(loop_num);
    std::vector<double *> h_Barray(loop_num);
    std::vector<double *> h_Carray(loop_num);
    ...
    CUDA_CHECK(cudaMalloc(&d_Aarray, sizeof(double *) * loop_num));
    CUDA_CHECK(cudaMalloc(&d_Barray, sizeof(double *) * loop_num));
    CUDA_CHECK(cudaMalloc(&d_Carray, sizeof(double *) * loop_num));
    CUDA_CHECK(
        cudaMemcpy(d_Aarray, h_Aarray.data(), sizeof(double *) * loop_num, cudaMemcpyHostToDevice));
    CUDA_CHECK(
        cudaMemcpy(d_Barray, h_Barray.data(), sizeof(double *) * loop_num, cudaMemcpyHostToDevice));
    CUDA_CHECK(
        cudaMemcpy(d_Carray, h_Carray.data(), sizeof(double *) * loop_num, cudaMemcpyHostToDevice));

    time = 0, temp_time = 0;
    for (int i{0}; i < NUM_WARPUP; ++i) {
        CUBLAS_CHECK(cublasDgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, nm, nn, k, &one,
    ...
    std::cout << "[cublas batched dgemm] " << "m: " << m << ", n: " << n << ", k: " << k << ", "
              << "nm: " << nm << ", nn: " << nn << ", "
              << "latency: " << time << " ms, "
              << "Effective TFLOPS: " << 2.0 * m * n * k / time / 1e9 << " TFLOPS, " << std::endl;

    // thrust::device_ptr<double> thrust_d_C1(d_C1);
    // thrust::device_ptr<double> thrust_d_C2(d_C2);
