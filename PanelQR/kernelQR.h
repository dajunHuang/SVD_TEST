...
#pragma unroll
      ...
      half norm_x_squre = warpAllReduceSum(nu);
      half norm_x       = hsqrt(norm_x_squre);

      // 1、求u=x/norm(x);
      // T scale = 1.0 / norm_x;
      half scale = __float2half(1.0) / norm_x;
#pragma unroll
      for (long k = 0; k < rowDataNum; k++)
      {
        if (i + k * blockDim.x < mm && i + k * blockDim.x >= cols)
